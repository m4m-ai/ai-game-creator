...
export enum LoadType {
    ...
    JSON = 1,
    IMAGE = 2,//图片
    assetMgr = 3,//用 m4m.framework.assetMgr  暂时加上 因加载UI时部分处理 不用这个会有问题 后续查
}
...
export class LoaderManage {
    ...
    public init(CDNURL: string) {
        LoaderManage._CDNURL = CDNURL;
    }

    /*
     *  priority 优先级,数值越高越优先
     */
    public load(url: string, onLoadFinished: Function, loadType: LoadType = LoadType.ARRAYBUFFER, priority: number = 0): Loader {
        let loader: Loader;
    ...
    *  priority 优先级,数值越高越优先
    */
    public loadList(urlList: string[], onLoadFinished: Function, priority: number = 0): FilesLoader {
        let fileLoad: FilesLoader;
        urlList.forEach((url) => {
            let type = LoadType.ARRAYBUFFER;
            if (url.endsWith("_Json.json")) {
                type = LoadType.JSON;
            }
            fileLoad.add(url, type, priority);
        });
        fileLoad.start(onLoadFinished);
        return fileLoad;
    }
