...
import { AssembleUIReadTool } from "./AssembleUIReadTool";
import { LoaderManage } from "./LoaderManage";
import { UiManager } from "../PSDUI/UiManager";

export enum AssembleCreatUIType {
    all = 0,
    noClip = 1,
    noMesh = 2,
}
//组装UI类 原小代写的 testCreat 类
export class AssembleCreatUI {
     ...
     */
    public static init(systemQuality: SystemQualityType, systemOS: PlatformType, carePathReplaceMap: { [srcPath: string]: string }) {
    ...
    public static getFilesByType(path: string, fileName: string, type: AssembleCreatUIType, callBack: Function) {
        if (!AssembleCreatUI.isSetGetMesh) {
            AssembleCreatUI.isSetGetMesh = true;
            AssembleCreatUI.setGetMesh();
        }
        let readFileList: string[] = [];
        switch (type) {
            case AssembleCreatUIType.noClip:
                readFileList.push(path + fileName + "/resources/" + fileName + AssembleCreatUI.noClipEnd);
                break;
            case AssembleCreatUIType.noMesh:
                readFileList.push(path + fileName + "/resources/" + fileName + AssembleCreatUI.noMeshEnd);
                readFileList.push(path + fileName + "/resources/" + fileName + AssembleCreatUI.meshEnd);
                break;
            default:
                readFileList.push(path + fileName + "/resources/" + fileName + AssembleCreatUI.allEnd);
        }
        // console.error("开始加载   1111111111111111111111    " + fileName + "              ");
        // let resList:Array<CResBundlesData>=new Array();
        // let fileLoad = new FilesLoader();
        ...
        // });
        //加载一个url列表
        LoaderManage.Instance.loadList(readFileList, (v) => {
            let info = [];
            v.forEach((element) => {
                if (element) {
                    if (!element.length) {
                        ...
                        let bd = AssembleUIReadTool.readInfoByBuffer(url, element);
                        // console.error(info);
                        info = info.concat(bd);
                    } else {
    ...
    public static set systemQuality(v: number) {
        switch (v) {
            case 0:
                AssembleCreatUI.qualityString = "LOW_";
                break;
            case 1:
                AssembleCreatUI.qualityString = "MID_";
                break;
            default:
                AssembleCreatUI.qualityString = "";

        }
    }

    public static set systemOS(v: number) {
        AssembleCreatUI._systemOS = v;
        switch (v) {
            case 1:
                AssembleCreatUI.osString = "_10x10.astc.bin.js";
            ...
            case 2:
                ...
                break;
            default:
                AssembleCreatUI.osString = "";

        }
    }
    public static disposeResByUrl(path: string) {
        if (AssembleCreatUI.pfbResList.has(path)) {
            let aaa = AssembleCreatUI.pfbResList.get(path);
            aaa.forEach((v) => {
                if (v.dispose) {
                    v.dispose();
                }
            ...
            aaa = null;
            AssembleCreatUI.pfbResList.delete(path);
        }
        if (AssembleCreatUI.pfbList.has(path)) {
            AssembleCreatUI.pfbList.get(path)
                .dispose();
            AssembleCreatUI.pfbList.delete(path);
        }
    }
    ...
    public static async createPfb(path: string, filePath: string) {
        ...
        if (AssembleCreatUI.pfbList.get(savePath)) {
            return AssembleCreatUI.pfbList.get(savePath);
        }

        let temp;
        if (!this.loadMap.get(path + filePath)) {
            temp = AssembleCreatUI.creatTranByBuffer(path, filePath);
        ...
        pfb.apply(obj);
        AssembleCreatUI.pfbList.set(savePath, pfb);
        this.loadMap.delete(path + filePath);

        //替换 getCloneTrans接口 ， 稳定后移入引擎
        pfb.getCloneTrans = () => {
            let t: m4m.framework.transform;
            t = AssembleCreatUI.createTran(savePath) as m4m.framework.transform;
    ...
    public static async createPfb2D(path: string, filePath: string) {
        ...
        let obj = await AssembleCreatUI.creatTran2DByBuffer(path, filePath);
    ...
    public static async creatTranByBuffer(path: string, filePath: string) {
        let dataList = [];
        let savePath = path + filePath + "/";
        let urlMap = AssembleCreatUI.pfbResList.get(savePath);
        if (!urlMap) {
            urlMap = new cMap<any>();
            AssembleCreatUI.pfbResList.set(savePath, urlMap);
        }
        ...
        if (!list) {
            ...
            temp = new Promise((resolve: ((a) => void)) => {
                AssembleCreatUI.getFilesByType(path, filePath, 2, resolve);
            });
            //     this.loadMap.set(path + filePath, temp);
            // } else {
        ...
        return await AssembleCreatUI.create(savePath) as m4m.framework.transform;
    }
    public static async creatTran2DByBuffer(path: string, filePath: string) {

        let dataList = [];
        let savePath = path + filePath + "/";
        if (!AssembleCreatUI.pfbResList.get(savePath)) {
            AssembleCreatUI.pfbResList.set(savePath, new cMap<any>());
        }
        if (!AssembleCreatUI.pfbResList.get(savePath)
            .get(savePath)) {
            ...
            dataList = await temp;
            AssembleCreatUI.pfbResList.get(savePath)
                .set(savePath, dataList);
        }
        return await AssembleCreatUI.create(savePath) as m4m.framework.transform2D;
    ...
    public static async create(path: string) {
        ...
        // }
        let pfbres = AssembleCreatUI.pfbResList.get(path);
        ...
        if (!isTran2D) {
            ...
            if (dataMap["Prefab2D"] && dataMap["Prefab2D"].length > 0) {
                ...
                let trans = AssembleCreatUI.makeAPrefab2D(dataMap["Prefab2D"][0], insidMap);
                AssembleCreatUI.setCompsToTran2D(trans, dataMap["Prefab2D"][0], insidMap, compMap, path);
                AssembleCreatUI.referenceComps(insidMap, compMap);
                dataMap = null;
    ...
    private static referenceComps(insidMap, compMap) {
        for (let index = 0; index < compMap.length; index++) {
            ...
            AssembleCreatUI.referenceComp(element, insidMap);
        }
    ...
    private static imgPathFix(path: string, imageName: string) {
        ...
        if (AssembleCreatUI.osString && AssembleCreatUI.fileList.indexOf(imageName) == -1 && !fileName.startsWith("Lightmap")) {
            if (imageName.endsWith(".png")) {
                fileName = fileName.replace(".png", AssembleCreatUI.osString);
            } else if (imageName.endsWith(".jpg")) {
                fileName = fileName.replace(".jpg", AssembleCreatUI.osString);
            }
        }
        let url = "";
        if (!fileName.startsWith("Lightmap")) {
            url = path + "resources/" + AssembleCreatUI.qualityString + fileName;
        } else {
    ...
    private static makeAPrefab2D(pfInfo, insidMap) {
        ...
        if (pfInfo.children) {
            for (let i = 0; i < pfInfo.children.length; i++) {
                let childTranInfo = pfInfo.children[i];
                let childTran = AssembleCreatUI.makeAPrefab2D(childTranInfo, insidMap);
                trans.addChild(childTran);
            }
        }
        return trans;
    ...
    private static setCompsToTran2D(trans: m4m.framework.transform2D, pfInfo, insidMap, compMap, path: string) {
        ...
        for (let i = 0; i < pfInfo.components.length; i++) {
            ...
            let rawComp = AssembleCreatUI.makeAComp2D(trans, compInfo, insidMap, compMap, path);
            if (rawComp) {
        ...
        for (let i = 0; i < trans.children.length; i++) {
            let childTran = trans.children[i];
            let childTranInfo = pfInfo.children[i];
            AssembleCreatUI.setCompsToTran2D(childTran, childTranInfo, insidMap, compMap, path);
        }
    }
    // //组装动画片段
    ...
    private static makeAComp2D(trans: m4m.framework.transform2D, compInfo: any, insidMap, compMap, path: string) {
        ...
        switch (name) {
            ...
            case "label":
                ...
                //compLabel.font = UiManager.getFont(compInfo.fontsize);
    ...
    // }
    //组装Mesh
    // private static createMesh(meshData: Mesh, webgl: WebGLRenderingContext): m4m.framework.mesh {
...
if (m4m) {
    ...
    m4m["test"].testCreat = AssembleCreatUI;
}