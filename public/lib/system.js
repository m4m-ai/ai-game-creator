...
System.modelInit = function (key, paths, fails) {
    ...
    for (let i = 0, len = model.____imports____.length; i < len; ++i) {
        ...
        if (!cmodel) { //如果不在system模块, 那就在commonjs中找
            if (!System.CommonJsCache) {
                continue;
            }
            if (!ckey.startsWith("m4m_editor")) {
                continue;
            }

            let nodeJsPath = ckey.substring(10);
            cmodel = System.CommonJsCache[nodeJsPath];
            if (!cmodel) {
                let commonJsPath = "./src" + nodeJsPath;
                cmodel = System.CommonJsCache[commonJsPath + ".ts"];
                if (!cmodel) {
                    cmodel = System.CommonJsCache[commonJsPath + ".tsx"];
                    if (!cmodel) {
                        cmodel = System.CommonJsCache[commonJsPath + ".jsx"];
                        if (!cmodel) {
                            continue;
                        }
                    }
