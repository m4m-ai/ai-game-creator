...
export class WalletErrLog extends ExcelDataBase{
	...
	*/
	public static getlist:(offset?:number, count?:number) => Promise<WalletErrLog>;
...
var baseData:WalletErrLog = new WalletErrLog ();	
...
public static  clone(old:WalletErrLog):WalletErrLog{
var clone :WalletErrLog = new WalletErrLog();
	clone.id=old.id;
...
public   clone(old:WalletErrLog){
	...
	public static add(a: WalletErrLog, b: WalletErrLog, start: number = 0, end: number, limit: WalletErrLog) {
		if(!a || !b) return null;
	...
	public static sub(a: WalletErrLog, b: WalletErrLog, start: number = 0, end: number) {
		if(!a || !b) return null;
		let result = this.clone(a);
		for(let i = Math.max(start, 0), e = Math.min(end, this.params.length); i < e; i++) {
			const par = this.params[i];
	...
	public static random(src: WalletErrLog, i: number = 0) {
		if(src[this.params[i]] == 0) // NOTE:
			src[this.params[i]] = Math.random();
		return JSON.stringify(src);
	}

	public static large(a: WalletErrLog, b: WalletErrLog, i: number = 0) {
		return a[this.params[i]] > b[this.params[i]];
	}

	public static max(a: WalletErrLog, b: WalletErrLog, i: number = 0) {
		if(a[this.params[i]] > b[this.params[i]])
			return a;
		return b;
	}

	public static json(a: WalletErrLog, data) {
	...
	public static setProperty(a: WalletErrLog, p: number, value) {
...
}if(!m4m.__ExcDate__)m4m.__ExcDate__= { } ; if(!m4m.__ExcDate__.__list) m4m.__ExcDate__.__list = []; m4m.__ExcDate__.__list.push(WalletErrLog);